...
def get_kl_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    # 为14个类别创建IoU阈值
    # 规则：车辆类使用0.7阈值，小物体(如Cone)或复杂物体(如行人)使用0.5阈值
    large_vehicle_classes = [1, 2, 3, 4, 5, 6, 7, 10, 11, 12, 13]  # 大型车辆类使用0.7
    small_object_classes = [0, 8, 9]  # 行人、杂项车辆和锥筒使用0.5

    # 创建14类的IoU阈值矩阵
    overlap_values_strict = []
    overlap_values_loose = []

    for i in range(14):
        if i in large_vehicle_classes:
            overlap_values_strict.append(0.7)
            overlap_values_loose.append(0.5)  # 宽松标准
        else:
            overlap_values_strict.append(0.5)  # 小物体使用0.5
            overlap_values_loose.append(0.25)  # 宽松标准

    # 为三种难度级别创建阈值矩阵
    overlap_0_7 = np.array([overlap_values_strict] * 3)  # 3个难度级别
    overlap_0_5 = np.array([overlap_values_strict, overlap_values_loose, overlap_values_loose])  # 不同难度级别的阈值
    
    # 堆叠两种评估标准
    min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)  # [2, 3, 14]
