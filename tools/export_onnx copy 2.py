...
def main():
    ...
    # dummy_voxels = torch.rand(10000, 32, 4)        # [M, T, 4]
    # dummy_voxel_idxs = torch.randint(0, 10, (10000, 4))  # [M, 4]
    # dummy_voxel_num = torch.randint(1, 32, (10000,))     # [M]
    # batch_size = 1

    # # 8. **转换 NumPy 格式输入**
    onnx_input = dict()
    onnx_input['voxels'] = dummy_voxels.cpu().numpy().astype(np.float32)
    onnx_input['voxel_num'] = dummy_voxel_num.cpu().numpy().astype(np.float32)      # 👈
    onnx_input['voxel_idxs'] = dummy_voxel_idxs.cpu().numpy().astype(np.float32)    # 👈
    ...
    torch_output=result=model(dummy_input)
    aaaa=1
