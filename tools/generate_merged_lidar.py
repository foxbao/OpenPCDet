...
def save_point_cloud_to_pcd(points, output_path):
    ...
    if points.shape[1] >= 4:
        ...
        pcd.colors = o3d.utility.Vector3dVector(
            np.tile(intensities[:, np.newaxis], (1, 3))
        )  # 将强度映射到 RGB
...
if __name__ == "__main__":
    ...
    lidars.append(
        {
            "path": "../data/kl/v1.0-trainval/sample/s1/perception_1104_blue_igv_02-century02_07/lidar/helios_front_left/1730682634401.bin",
            "extrinsic": np.array(
                [
                    7.336647033691406,
                    1.4024821519851685,
    ...
    lidars.append(
        {
            "path": "../data/kl/v1.0-trainval/sample/s1/perception_1104_blue_igv_02-century02_07/lidar/helios_rear_right/1730682634401.bin",
            "extrinsic": np.array(
                [
                    -7.255270481109619,
                    -1.6236361265182495,
    ...
    if point_clouds:  # 如果列表不为空
        ...
        print("Points shape:", merged_point_cloud.shape)
        print("Min x:", merged_point_cloud[:, 0].min())
        print("Max x:", merged_point_cloud[:, 1].max())
